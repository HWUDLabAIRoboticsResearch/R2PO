...
def run_training_loop(
    ...
    if resolved_policy_variant is None:
        if hidden_dim is not None and latent_dim_layer1 is not None and latent_dim_layer2 is not None:
            ...
            resolved_policy_variant = "random_feature_tanh"
        else:
            resolved_policy_variant = "linear"

    resolved_projection_seed = projection_seed
    if resolved_policy_variant == "random_feature_tanh":
        if feature_activation not in (None, "tanh"):
            raise ValueError(
                f"Only tanh random features are currently supported, got {feature_activation}"
            )
        if resolved_projection_seed is None:
            run_name = os.path.basename(logdir.rstrip("/"))
            match = re.match(r"run_(\d+)$", run_name)
            run_idx = int(match.group(1)) if match else 1
            resolved_projection_seed = int(random_feature_seed_base or 0) + run_idx
    elif resolved_policy_variant == "projected_mlp_qr" and resolved_projection_seed is None:
        if activation != "tanh":
            raise ValueError(
                f"Only tanh projected MLP policies are currently supported, got {activation}"
            )
        run_name = os.path.basename(logdir.rstrip("/"))
        match = re.match(r"run_(\d+)$", run_name)
        run_idx = int(match.group(1)) if match else 1
    ...
    if run_mode == "resume":
        start_episode = restore_agent_from_run(
            ...
            logdir,
            semantic=True,
            is_qtable=(task == "dist_state_llm_num_optim_semantics"),
