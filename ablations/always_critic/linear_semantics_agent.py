...
from ablations.always_critic.context_builder import (
    build_critic_llm_reflection_context,
    format_trajectory_summary,
)
from ablations.always_critic.llm_brain_reflective import (
    ReflectiveLLMBrain,
)
...
class ReflectiveAlwaysCriticLinearSemanticsAgent(LLMNumOptimSemanticAgent):
    def __init__(
        self,
        *args,
        jinja2_env,
    ...
    def train_policy(self, world, logdir):
        def parse_parameters(input_text):
            ...
            pattern = re.compile(r"params\[(\d+)\]:\s*([+-]?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)")
            matches = pattern.findall(input_text)
            # First-occurrence-wins: reasoning lines may repeat params[i] references;
            # we want the value from the proposal line, not the explanation.
            indexed_results = {}
        ...
        print("AlwaysCritic: Phase 1 - Search-LLM proposing params...")
        try:
        ...
        print("AlwaysCritic: evaluating proposal (all rollouts)...")
        ...
        print("AlwaysCritic: Phase 2 - Critic-LLM reflecting...")
        ...
        print("AlwaysCritic: evaluating revised proposal (all rollouts)...")
        ...
        # Always commit the reflected revision, regardless of whether it beat the
        # initial proposal. This isolates the contribution of the keep-best rule.
        winner = "revision_forced"
        best_params = revised_params
        best_reward = revised_reward
        winning_traj_buffer = self._revised_traj_buffer

        print(
            "Selection rule: always keep Critic-LLM revision "
            f"(reward={best_reward:.4f}, initial={initial_reward:.4f})"
        )
