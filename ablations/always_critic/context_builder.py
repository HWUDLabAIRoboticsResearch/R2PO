...
def build_critic_llm_reflection_context(
    ...
    This includes:
    - The initial proposal (params + reward)
    - Trajectory summary from the initial rollout
    - Rollout statistics
    - Optimization history for reference
    - Environment description
    """
    params_str = "; ".join(
        f"params[{i}]: {_format_number(v)}" for i, v in enumerate(
            initial_params.reshape(-1).tolist()
        )
    )
