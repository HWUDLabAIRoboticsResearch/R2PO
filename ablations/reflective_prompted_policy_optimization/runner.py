...
from ablations.reflective_prompted_policy_optimization.linear_semantics_agent import (
    ReflectivePromptedPolicyOptimizationLinearSemanticsAgent,
)
from ablations.reflective_prompted_policy_optimization.q_table_semantics_agent import (
    ReflectivePromptedPolicyOptimizationQTableSemanticsAgent,
)
...
def run_training_loop(
    ...
    critic_llm_conservative_threshold=None,
    rerun=None,
    **_extra,
):
    assert task in [
        "dist_state_reflective_prompted_policy_optimization",
        "cont_state_reflective_prompted_policy_optimization",
    ]
    ...
    if task == "dist_state_reflective_prompted_policy_optimization":
        ...
        agent = ReflectivePromptedPolicyOptimizationQTableSemanticsAgent(
            logdir,
            dim_actions,
            dim_states,
            max_traj_count,
        ...
        agent = ReflectivePromptedPolicyOptimizationLinearSemanticsAgent(
            logdir,
            dim_actions,
            dim_states,
            max_traj_count,
    ...
    agent.critic_llm_conservative_threshold = critic_llm_conservative_threshold

    agent.max_iterations = num_episodes

    print("reflective_prompted_policy_optimization init done")
    ...
    agent.warmup_replay_count = len(agent.replay_buffer.buffer)
