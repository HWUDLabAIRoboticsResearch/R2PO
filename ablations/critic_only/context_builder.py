...
def format_trajectory_summary(traj_buffer, last_n=1):
    """Format the most recent trajectory(ies) as readable raw step-by-step text."""
    if traj_buffer is None or not getattr(traj_buffer, "buffer", None):
    ...
    for traj_idx, trajectory in enumerate(trajectories):
        ...
        lines.append(f"Trajectory {traj_idx} (length={len(steps)}):")
...
def build_critic_only_context(
    ...
    """Build the single-pass CriticOnly prompt context from history and trajectory."""
    return {
        ...
        "trajectory_summary": format_trajectory_summary(traj_buffer),
