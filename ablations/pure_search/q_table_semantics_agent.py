...
from ablations.pure_search.llm_brain_two_pass import (
    TwoPassLLMBrain,
)
...
class PureSearchQTableSemanticsAgent(LLMNumOptimQTableSemanticsAgent):
    """Pure search control: propose twice from the same history, evaluate both."""
    ...
    def train_policy(self, world, logdir):
        def parse_parameters(input_text):
            ...
            return np.array([indexed_results[i] for i in range(self.rank)]).reshape(
                (self.rank,)
            )
        ...
        print("Pure search: evaluating proposal A...")
        self.q_table.update_policy(proposal_a)
        proposal_a_results = []
        proposal_a_trajs = []
        ...
        proposal_a_traj_buffer = proposal_a_trajs[proposal_a_repr_idx]

        print("Pure search: evaluating proposal B...")
        self.q_table.update_policy(proposal_b)
        proposal_b_results = []
        proposal_b_trajs = []
        ...
        if proposal_b_reward >= proposal_a_reward:
            ...
            winning_traj_buffer = proposal_a_traj_buffer
